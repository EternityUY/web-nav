...
export default function LinkCard({ link }: LinkCardProps) {
  return (
    <a
      ...
      className="group relative block rounded-xl overflow-hidden transition-all duration-200 hover:scale-[1.02]"
    >
      {/* Pin badge */}
      {link.pinned && (
        <Pin size={12} className="absolute -top-1 -right-1 text-yellow-400/70 rotate-45 z-20" />
      )}

      {/* Liquid glass layers — pure CSS, no per-instance JS */}
